...
export default class DefaultTransceiverController
  ...
  private setupAudioRedWorker(): void {
    ...
    if (supportsRTCScriptTransform) {
      ...
      this.logger.warn('[AudioRed] Encoded insertable streams not supported');
      return;
    }
    ...
    try {
      ...
      this.logger.info(
        `[AudioRed] Recreating peer connection and renegotiating with audio redundancy disabled`
      );
